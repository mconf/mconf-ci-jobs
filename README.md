...
| **all-py-bandit** | Scan | Python, Bandit | | ✓ | | | | Configurable severity/confidence, JSON+screen report artifact, PR job summary |
...
* `portal-ruby-tests.yml`
  Runs Rails tests with MySQL and Redis services, generates a SimpleCov coverage report and posts it as a PR comment via octocov (reads `coverage/.resultset.json`). Reads Ruby version from `.ruby-version` file. Requires OAuth and Devise secrets. Coverage rules can be customized per repo with a `.octocov.yaml`; otherwise the workflow applies a default.
...
* `all-py-bandit.yml`
  Runs Bandit (Python SAST) over a source tree and fails the build on findings at or above the configured severity and confidence. Publishes a JSON and screen report as an artifact and a findings table in the job summary.
  **Usage:** See [`examples/all-py-bandit.yml`](examples/all-py-bandit.yml)
* `all-trufflehog.yml`
